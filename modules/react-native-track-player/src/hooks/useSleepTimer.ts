...
import type { SleepTimerChangedEvent } from '../interfaces';
...
export function useSleepTimer(): SleepTimerState {
  ...
  const startCountdown = useCallback(
    (endTime: number) => {
      ...
      const tick = () => {
        ...
        setState((prev) => (prev.remaining === remaining ? prev : { ...prev, remaining }));
      };
  ...
  useTrackPlayerEvents(
    [Event.SleepTimerChanged, Event.SleepTimerComplete],
    (event) => {
      if (event.type === Event.SleepTimerChanged) {
        const payload = event as unknown as SleepTimerChangedEvent & { type: Event };
        if (payload.active) {
          if (payload.endOfTrack) {
            ...
            setState({
              ...
              remaining: Math.max(
                0,
                Math.round(payload.endTime - Date.now() / 1000)
              ),
              endOfTrack: false,
              completed: false,
            });
